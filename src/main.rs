...
async fn main() {
  ...
  daemon = daemon.args(["config", "--json",
    "--repo-dir", REPO_PATH,
    "Addresses.API", &("[\"/ip4/127.0.0.1/tcp/".to_owned() + &get_api_port().to_string() + "\"]")]);
  ...
  daemon = daemon.args(["config", "--json",
    "--repo-dir", REPO_PATH,
    "Addresses.Swarm", &("[\"/ip4/0.0.0.0/tcp/".to_owned() + swarm_port_str + "\",\"/ip6/::/tcp/" + swarm_port_str + "\",\"/ip4/0.0.0.0/udp/" + swarm_port_str + "/quic\", \"/ip6/::/udp/" + swarm_port_str + "/quic\"]")]);
  daemon.output().unwrap();
